    ...
    <div className="space-y-4" data-testid="challenge-details">
          ...
          {challenge.state === 2 && challenge.challengerVote !== 0 && challenge.participantVote !== 0 && challenge.challengerVote === challenge.participantVote && (
            <>
              ...
              <div className="flex justify-between items-center">
                ...
                <Badge variant="outline" className="text-yes border-yes/30" data-testid="badge-winner">
      ...
      {challenge.state === 1 && joined && address && (() => {
        ...
        if ((isChallenger || isParticipant) && myVote === 0) {
          return (
            <div className="flex items-center gap-2.5 p-3 rounded-md border border-warning/30 bg-warning/5" data-testid="vote-nudge-banner">
              <AlertTriangle className="w-4 h-4 text-warning shrink-0" />
              <div>
                <p className="text-xs font-bold text-warning">Your vote is needed</p>
        ...
        <div className="rounded-md border-2 border-[hsl(var(--primary))]/40 bg-[hsl(var(--primary))]/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex flex-col items-center gap-1.5 p-4 rounded-md border-2 border-yes/40 bg-yes/10 hover-elevate disabled:opacity-50"
            >
              {actionLoading === 'Vote: I Won' ? <Loader2 className="w-6 h-6 animate-spin text-yes" /> : <ThumbsUp className="w-6 h-6 text-yes" />}
              <span className="text-sm font-bold text-yes">I Won</span>
            </button>
            <button
              data-testid="button-vote-lost"
              onClick={() => confirmVote(false)}
              disabled={!!actionLoading}
              className="flex flex-col items-center gap-1.5 p-4 rounded-md border-2 border-no/40 bg-no/10 hover-elevate disabled:opacity-50"
            >
              {actionLoading === 'Vote: Opponent Won' ? <Loader2 className="w-6 h-6 animate-spin text-no" /> : <ThumbsDown className="w-6 h-6 text-no" />}
              <span className="text-sm font-bold text-no">Opponent Won</span>
            </button>
          </div>
        </div>
      )}
      ...
      {challenge.state === 0 && joinExpired && !joined && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 p-3 rounded-md border border-warning/30 bg-warning/5">
            <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0" />
            <p className="text-xs text-warning">Join deadline passed with no opponent. Creator can reclaim funds.</p>
          </div>
      ...
      {challenge.state === 1 && challenge.challengerVote !== 0 && challenge.participantVote !== 0 && challenge.challengerVote !== challenge.participantVote && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 p-3 rounded-md border border-warning/30 bg-warning/5">
            <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0" />
            <p className="text-xs text-warning">Votes conflict - creator and opponent disagree on the outcome. Both parties can claim a refund.</p>
          </div>
          <GasEstimate estimateFn={() => estimateGas('issueRefund', [BigInt(betId)])} ethUsd={ethUsd} address={address} />
      ...
      {challenge.state === 1 && resolveExpired && !(challenge.challengerVote !== 0 && challenge.participantVote !== 0 && challenge.challengerVote !== challenge.participantVote) && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 p-3 rounded-md border border-warning/30 bg-warning/5">
            <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0" />
            <p className="text-xs text-warning">Resolve deadline passed without agreement. Both parties can claim a refund.</p>
          </div>
          <GasEstimate estimateFn={() => estimateGas('issueRefund', [BigInt(betId)])} ethUsd={ethUsd} address={address} />
    ...
    <div className="space-y-4" data-testid="offer-details">
      <div className="rounded-md border border-border bg-muted/30 p-4">
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            {offer.paid && <Badge variant="outline" className="text-yes border-yes/30">Paid</Badge>}
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-4 mb-3 py-2">
          <div className="text-center">
            <div className={`text-2xl font-bold ${offer.creatorSideYes ? 'text-yes' : 'text-no'}`}>
          ...
          {offer.state === 2 && offer.creatorVote !== 0 && offer.takerVote !== 0 && offer.creatorVote === offer.takerVote && (
            <>
              ...
              <div className="flex justify-between items-center">
                ...
                <Badge
                  variant="outline"
                  className={`${offer.creatorVote === 1 ? 'text-yes border-yes/30' : 'text-no border-no/30'}`}
        ...
        <div className="rounded-md border-2 border-[hsl(var(--primary))]/40 bg-[hsl(var(--primary))]/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex flex-col items-center gap-1.5 p-4 rounded-md border-2 border-yes/40 bg-yes/10 hover-elevate disabled:opacity-50"
            >
              {actionLoading === 'Vote: YES' ? <Loader2 className="w-6 h-6 animate-spin text-yes" /> : <TrendingUp className="w-6 h-6 text-yes" />}
              <span className="text-sm font-bold text-yes">YES Won</span>
            </button>
            <button
              ...
              className="flex flex-col items-center gap-1.5 p-4 rounded-md border-2 border-no/40 bg-no/10 hover-elevate disabled:opacity-50"
            >
              {actionLoading === 'Vote: NO' ? <Loader2 className="w-6 h-6 animate-spin text-no" /> : <TrendingDown className="w-6 h-6 text-no" />}
              <span className="text-sm font-bold text-no">NO Won</span>
            </button>
          </div>
        </div>
      )}
      ...
      {offer.state === 0 && joinExpired && !hasTaker && !offer.paid && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 p-3 rounded-md border border-warning/30 bg-warning/5">
            <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0" />
            <p className="text-xs text-warning">Join deadline passed with no taker. Creator can reclaim funds.</p>
          </div>
      ...
      {offer.state === 1 && offer.creatorVote !== 0 && offer.takerVote !== 0 && offer.creatorVote !== offer.takerVote && !offer.paid && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 p-3 rounded-md border border-warning/30 bg-warning/5">
            <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0" />
            <p className="text-xs text-warning">Votes disagree - creator and taker voted differently. Both parties can claim a refund.</p>
          </div>
          <GasEstimate estimateFn={() => estimateGas('refundOffer', [BigInt(betId)])} ethUsd={ethUsd} address={address} />
          <Button data-testid="button-refund" onClick={() => confirmRefund('Vote conflict')} disabled={!!actionLoading} variant="outline" className="w-full" size="lg">
            {actionLoading === 'Refund' ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <RefreshCw className="w-4 h-4 mr-2" />}
